...
class Navbar extends Component {
  constructor(props) {
    super(props);
    this.state = {
      // Dynamic state for dropdown toggles
      dropdownStates: {},
    };
  }
  ...
  toggleDropdown = (menuId) => {
    this.setState((prevState) => ({
      dropdownStates: {
        ...
        [menuId]: !prevState.dropdownStates[menuId],
      },
    }));
  };
  ...
  renderMenuItems = () => {
    ...
    const { dropdownStates } = this.state;
      ...
      if (item.subItems) {
        return (
          <li key={index} className="nav-item dropdown">
            ...
            <div
              className={classname("dropdown-menu", {
                show: dropdownStates[menuId],
              })}
              aria-labelledby={`topnav-${menuId}`}
            >
      ...
      return (
        <li key={index} className="nav-item">
          <Link
            className="nav-link"
            to={item.path}
            onClick={(e) => this.activateParentDropdown(e.currentTarget)}
          >
            <i className={`${item.icon} me-2`}></i>
            {t(item.title)}
          </Link>
          ...
          <Container fluid>
            ...
            >
              <Collapse
                ...
                id="topnav-menu-content"
              >
                <ul className="navbar-nav">{this.renderMenuItems()}</ul>
              </Collapse>
            </nav>
          </Container>
        </div>
